...
def load_tic_data(TIC: int, sector: int = -1):
    ...
    FNAME = DATA_DIR
    if sector == -1:
        FNAME += "%d_sector_all.txt" % TIC; print("Loading all lightcurves of TIC %d" % TIC)
    else:
...
def main():
    ...
    new_drifting_sectors = [[17, 18, 58], [10, 11, 63, 64], [19, 43, 44, 45, 59], [19, 43, 44, 45, 59],
                            [16, 17, 56, 57]]
    paper_drifting = [115394297, 178739533, 239714064, 240918551, 283539216, 283839379, 305454334, 336538437, 378275980,
                      391244527, 444555685, 451708707]
